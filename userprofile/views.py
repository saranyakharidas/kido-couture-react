...
def test(request):
    # Get the referral code entered by the user
    if request.method == 'POST':
        referral_code = request.POST.get('code')
        
        # Check if the entered referral code is valid
        try:
        ...
        if referred_user:
            ...
            referred_user_wallet = wallet.objects.get(user=referred_user.user)
    ...
    # Handle the case where the Referral object does not exist for the user
        refferal = None  # You can set it to None or another default value
